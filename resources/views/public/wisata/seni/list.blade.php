@extends('public.layout.app', ['title' => 'Kesenian dan Budaya'])
@section('meta')
  <meta name="description" content="Kesenian dan Budaya" />
  <meta name="keywords" content="Diskominfo Subang" />
  <meta property="og:title" content="Kesenian dan Budaya"/>
  <meta property="og:type" content="Kesenian dan Budaya"/>
            ...
            <div class="row">
              ...
              @foreach ($budaya as $data)
                <div class="col-sm-12 col-md-6 col-lg-6">
                  <div class="blog-item">
                    <div class="blog__img">
                      <a href="{{route('public.seni.detail',['slug'=>$data->slug])}}">
                        <img height="190px" width="190px"  src="{{($data->img) ? asset('/storage/seni/images/'.$data->img) : asset('backend/img/default.png')}}" alt="{{$data->title}}">
                      </a>
                    </div><!-- /.entry-img -->
                    <div class="blog__content">
                      <h4 class="blog__title"><a href="{{route('public.seni.detail',['slug'=>$data->slug])}}">{{$data->title}}</a></h4>
                      <a href="{{route('public.seni.detail',['slug'=>$data->slug])}}" class="btn btn__secondary btn__link">Selengkapnya</a>
                    </div><!-- /.entry-content -->
                  </div><!-- /.blog-item -->
                </div><!-- /.col-lg-4 -->
              @endforeach
